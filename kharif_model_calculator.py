...
class Budget:
	...
	def summarize(self, crops, start_date_index, end_date_index , monsoon_end_date_index, circle_name):
		...
		self.sm_crop_end = np.array([self.sm[crops[i].end_date_index[circle_name]][i]	for i in range(len(crops))])
 		self.sm_monsoon_end = np.array([self.sm[monsoon_end_date_index+1][i]	for i in range(len(crops))])
		self.runoff_crop_end = [np.sum(self.runoff[start_date_index:crops[i].end_date_index[circle_name] + 1,i]) + np.sum(self.sec_run_off[start_date_index:crops[i].end_date_index[circle_name]+1,i]) for i in range(len(crops))]
		...
		self.infil_crop_end = [np.sum(self.infil[start_date_index:crops[i].end_date_index[circle_name]+1,i]) - np.sum(self.sec_run_off[start_date_index:crops[i].end_date_index[circle_name]+1,i]) for i in range(len(crops))]
		...
		self.AET_crop_end = [np.sum(self.AET[start_date_index:crops[i].end_date_index[circle_name]+1,i])	for i in range(len(crops))]
		self.AET_monsoon_end = [np.sum(self.AET[start_date_index:monsoon_end_date_index+1,i])	for i in range(len(crops))]
		self.GW_rech_crop_end = [np.sum(self.GW_rech[start_date_index:crops[i].end_date_index[circle_name]+1,i])	for i in range(len(crops))]
		...
		self.PET_minus_AET_monsoon_end = np.array([crops[i].PET_sum_monsoon[circle_name] - self.AET_monsoon_end[i]	for i in range(len(crops))])
		self.PET_minus_AET_post_monsoon = np.array([(crops[i].PET_sum_cropend[circle_name] - self.AET_crop_end[i])-self.PET_minus_AET_monsoon_end[i]	for i in range(len(crops))])
		self.PET_minus_AET_crop_end = np.array([crops[i].PET_sum_cropend[circle_name] - self.AET_crop_end[i]		for i in range(len(crops))])
...
class Crop:
	def __init__(self, name):
		self.name = name
		self.PET = {};	self.end_date_index =dict(); self.PET_sum_monsoon =dict(); self.PET_sum_cropend = dict();	
	@property
...
class Point:
	...
	@property
	def district(self):	return self.container_polygons[BOUNDARY_LABEL]['District'].lower()
	@property
	def taluka(self):	return self.container_polygons[BOUNDARY_LABEL]['Taluka'].lower()
	@property
	...
	@rain_circle.setter # used only for assigning arbitrary circle to a point having rain_circle == '0'
	def rain_circle(self, circle):      self.container_polygons[BOUNDARY_LABEL][Circle] = circle
	@zone_polygon.setter
	...
	def run_model(self, rain, crops, start_date_index, end_date_index, monsoon_end_date_index,lulc):
		...
		self.budget.summarize(crops, start_date_index, end_date_index, monsoon_end_date_index, (self.district, self.taluka, self.rain_circle))
	...
	def aet(self, day, crops):
		...
		PETs = np.array([crop.PET[(self.district, self.taluka, self.rain_circle)][day]	if day <= crop.end_date_index	else 0	for crop in crops])
...
class KharifModelCalculator:
	...
	def __init__(self, path, et0, zones_layer, soil_layer, lulc_layer, slope_layer): #, cadastral_layer

		self.path = path

		print 'generating zone features dictionary'
		self.zones_layer = VectorLayer(zones_layer, BOUNDARY_LABEL)
		print 'generating soil features dictionary'
		self.soil_layer = VectorLayer(soil_layer, SOIL_LABEL)
		print 'generating lulc features dictionary'
		self.lulc_layer = VectorLayer(lulc_layer, LULC_LABEL)
		# print 'generating cadastral features dictionary'
	...
	def calculate(self,
		...
		self.set_container_polygon_of_points_for_layers(self.output_grid_points, [self.soil_layer, self.lulc_layer, self.zones_layer])#, self.cadastral_layer])
		...
		count = 0
		# print self.rain.keys()
		for point in self.output_grid_points:
			count += 1
			# if count < 1000: continue
			if count % 100 == 0:	print cluster, year, count, '/', len(self.output_grid_points)
			# if point.rain_circle == '0':
			# 	for k in self.rain.keys():
			# 		# print k
			# 		if (k[0], k[1]) == (point.district, point.taluka) and k[2] != '0':
			# 			point.rain_circle = k[2]
			# 	# self.rain[(point.district, point.taluka, point.rain_circle)] = self.rain[next(iter(self.rain.keys()))]
			# if point.rain_circle == '0':
				# print self.rain[(point.district, point.taluka, point.rain_circle)]
			if point.lulc_type in ['agriculture', 'fallow land']:
				point.run_model(self.rain[(point.district, point.taluka, point.rain_circle)]['daily_rain'], self.crops, start_date_index, end_date_index, monsoon_end_date_index, dict_lulc[point.container_polygons[LULC_LABEL][Desc].lower()])
			else:
				point.run_model(self.rain[(point.district, point.taluka, point.rain_circle)]['daily_rain'], [self.LULC_pseudo_crops[point.lulc_type]], start_date_index, end_date_index, monsoon_end_date_index, dict_lulc[point.container_polygons[LULC_LABEL][Desc].lower()] )
		...
		for zone_id in self.zone_points_dict_non_ag_missing_LU:
			for lulc in self.zone_points_dict_non_ag_missing_LU[zone_id]:
				...
				for point in self.zone_points_dict_non_ag_missing_LU[zone_id][lulc]:
					count += 1
					if count % 20 == 0:
						print 'non_ag_missing_LU', cluster, year, 'zone', zone_id, 'lulc', lulc, ' : ', count, '/', len(self.zone_points_dict_non_ag_missing_LU[zone_id][lulc])
					point.run_model(
						self.rain[(point.district, point.taluka, point.rain_circle)]['daily_rain'],
						[self.LULC_pseudo_crops[lulc]],
						start_date_index, end_date_index, monsoon_end_date_index,lulc
					)
		...
		for zone_id in self.zone_points_dict_ag_missing:
			...
			for point in self.zone_points_dict_ag_missing[zone_id]:
				count += 1
				if count % 20 == 0:
					print 'ag_missing', cluster, year, 'zone', zone_id, ' : ', count, '/', len(
						self.zone_points_dict_ag_missing[zone_id])
				point.run_model(
					self.rain[(point.district, point.taluka, point.rain_circle)]['daily_rain'],
				    self.crops, start_date_index, end_date_index, monsoon_end_date_index,'agriculture'
				)
		...
		for zone_id in self.zone_points_dict_current_fallow:
			...
			for point in self.zone_points_dict_current_fallow[zone_id]:
				count += 1
				if count % 20 == 0:
					print 'current_fallow', cluster, year, 'zone', zone_id, ' : ', count, '/', len(
						self.zone_points_dict_current_fallow[zone_id])
				point.run_model(
					self.rain[(point.district, point.taluka, point.rain_circle)]['daily_rain'],
					self.currnet_fallow, start_date_index, end_date_index, monsoon_end_date_index,'current fallow'
				)


		# For urgent request, only zone-wise budgets required
