...
class KharifModelOutputProcessor:
	...
	def compute_zonewise_budget(self, zone_points_dict,zone_points_dict_ag_missing, zone_points_dict_current_fallow, zone_points_dict_non_ag_missing_LU, zones_layer):
		...
		for zone_id in zone_points_dict:
			...
			self.zone_area_village[zone_name]['circle'] = (zones_layer.feature_dict[zone_id]['District'].lower(),
			                                               zones_layer.feature_dict[zone_id]['Taluka'].lower(),
			                                               zones_layer.feature_dict[zone_id][Circle].lower())
	...
	def output_zonewise_budget_to_csv(
			self,
			zonewise_budgets,
			crops,rabi_crop_names,
			currnet_fallow,
			pseudo_crops,
			zonewise_budget_csv_filepath,
			rain_sum,
			year,
		...
		rows.append(['Rainfall (mm)']
			+ [rain_sum[self.zone_area_village[ID]['circle']]['sum'] for ID in zonewise_budgets	if 'agricultural' in zonewise_budgets[ID] for i in range(len(crops))]
		...
		for village in data_by_village:
			file_name = "Village_wise_output_"+village+'_'+year+'_'+('_avg' if village_avg_year_dict[village] == year else '')+'.xls'
